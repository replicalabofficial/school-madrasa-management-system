...
public class DashboardService : IDashboardService
{
    ...
    public async Task<HeadOfficeDashboardDto> GetHeadOfficeDashboardAsync(
        DashboardFilter filter)
    {
        ...
        if (filter.BranchId.HasValue)
        {
            studentsQuery = studentsQuery.Where(
                x => x.BranchId == filter.BranchId.Value);
        ...
        // Date filter for financial records
        if (filter.DateFrom.HasValue)
        {
            incomeQuery = incomeQuery.Where(
                x => x.Date >= filter.DateFrom.Value);
        ...
        var totalSalaries = await salaryQuery
        ...
        var incomeRecords = await incomeQuery
            .Select(x => new
            {
                x.Date,
                x.Amount
        ...
        var expenseRecords = await expenseQuery
            .Select(x => new
            {
                x.Date,
                x.Amount
        ...
        var months = incomeRecords
            ...
            .Concat(
                expenseRecords.Select(
                    x => x.Date.ToString("yyyy-MM")))
            .Distinct()
            .OrderBy(x => x)
            .ToList();
        ...
        foreach (var branch in branches)
        {
            var branchIncome = await _context.Income
                ...
                .Where(x =>
                    ...
                    x.Date < filter.DateTo.Value.Date.AddDays(1))
                .SumAsync(x => (decimal?)x.Amount) ?? 0;
            ...
            branchFinancialOverview.Add(
                new BranchDashboardFinancialDto
                {
                    BranchId = branch.Id,
        ...
        return new HeadOfficeDashboardDto
        {
            ...
            NetBalance =
                totalIncome +
                (await feeQuery.SumAsync(
                    x => (decimal?)x.Amount) ?? 0) -
                totalExpenses -
                totalSalaries,
